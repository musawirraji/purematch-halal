...
// Editorial "About" — a full-height two-column split so it fits inside one
// viewport (it lives in a sticky stacking card, which clips anything past 100vh).
// Left: the founder portrait floor-to-ceiling with a ruled caption. Right: a
// ruled masthead, an oversized pull-quote, a drop-cap lede + signature/CTA, and
// a numbered principles rail. Collapses to a single stacked column on mobile;
// blocks reveal in view (IO-based Reveal).
export function AboutIntro() {
  return (
    <section className="pm-section pm-about">
      <div className="pm-about__inner">
        <Reveal className="pm-about__portrait" as="figure">
          <div className="pm-about__frame">
            <MediaFrame file={ABOUT_INTRO.file} label={ABOUT_INTRO.label} tone="sand" fill />
          </div>
        ...
        <div className="pm-about__content">
          ...
          <div className="pm-about__col">
            {ABOUT_INTRO.body.map((p, i) => (
              <Reveal delay={140 + i * 70} key={i}>
                <p className={`pm-about__p ${i === 0 ? "pm-about__p--lede" : ""}`}>{p}</p>
              </Reveal>
            ))}
            <Reveal delay={140 + ABOUT_INTRO.body.length * 70}>
